...
1. Which ingredient results in the highest average rating.
2. Are there any missing values, if so how many?
3. How do ingredients affect the rating?
4. Is there a linear relationship between quantity and rating?
5. Do units affect the rating of the recipe?
...
I discovered that there are 68 unique ingredients and 1273 unique recipes. There is a lot of misisng data for ratings (1010 NA values). But, the average rating for the cookies is 0.815.
...
```{r}
table <- table(cookie$Ingredient)
table <- prop.table(table)
table2<-sort(table, decreasing = TRUE)
head(table2,20)
sum(table2)
```

```{python}
ingredient_counts = cookies['Ingredient'].value_counts()
ingredients = ingredient_counts / len(cookies)
top_20 = ingredients.head(20)
print(top_20)
```
...
```{r}
rec_table <- table(cookie$Ingredient)
rec_table2 <- sort(rec_table)
rec_table3 <- rec_table2[rec_table2<20]
test<-toString(names(rec_table3))
test
```

```{python}
rec_tab = ingredient_counts[ingredient_counts < 20]
rec_str = ', '.join(rec_tab.index.tolist())
print(rec_str)
```
